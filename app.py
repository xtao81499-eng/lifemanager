...
# Manual habit toggle buttons
from datetime import datetime as _dt
_today_str = _dt.now().strftime("%Y-%m-%d")
for habit_name in manual_habits:
    checked = get_checked_dates(habit_name)
    is_done = _today_str in checked
    label = f"{'✓ ' if is_done else ''}{habit_name} — 今日打卡"
    if st.button(label, key=f"toggle_{habit_name}", type="primary" if not is_done else "secondary"):
